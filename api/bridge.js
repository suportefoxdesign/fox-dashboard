...
async function getToken() {
  ...
  const resp = await fetch(CS_BASE + '/oauth/v1/access-token', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ grant_type: 'client_credentials', client_id: CS_API_KEY, client_secret: CS_API_SECRET })
  });
  const data = await resp.json();
  if (!data.access_token) throw new Error('Token invalido: ' + JSON.stringify(data));
...
async function getTransacoesMes(token) {
  ...
  do {
    ...
    const data = await resp.json();
    const pix = (data.transactions || []).filter(t => {
      const val = Math.abs(t.brlAmount);
      const tipo = (t.transactionType && t.transactionType.subType || '').toLowerCase();
      const desc = (t.description || '').toLowerCase();
      return (tipo.includes('pix') || desc.includes('pix')) && t.brlAmount > 0 && val >= 19 && val <= 300;
    });
    todas = todas.concat(pix);
    nextKey = data.nextPageStartKey || null;
  } while (nextKey);
  return todas;
}
...
function processaMetricas(txns) {
  ...
  txns.forEach(t => {
    const val = Math.abs(t.brlAmount);
    const cat = classificaPIX(t.brlAmount);
    if (!cat) return;
    const dataStr = t.transactionDate.split('T')[0];
  ...
  m.meta = 700; m.progressoMeta = (m.hoje.total / m.meta) * 100;
  m.diaAtual = diaHoje; m.timestamp = new Date().toISOString();
  return m;
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const txns = await getTransacoesMes(token);
    return res.status(200).json(processaMetricas(txns));
  } catch (e) {
    console.error('Erro bridge Fox:', e);
